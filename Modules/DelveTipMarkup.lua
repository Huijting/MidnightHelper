...
function ns.ReportTravelHintForWaypoint(mapID, label, x, y, currentMap)
	...
	--- ⚠️ The same connectivity gate the leg uses (see RouteFirstToFlightPoint). Without it
	--- the chat line and the waypoint disagree: the leg would correctly refuse to route an
	--- impossible flight while this line still told the player to take it. Two halves of one
	--- answer must not be gated differently -- that is how Rob got four contradictory lines
	--- in a single action on 3 Sep.
	--- 📌 Only a hard `false` silences it; nil is "we do not know" and still speaks.
	local blocked = false
	if from and ns.FlightPathExists then
		blocked = (ns.FlightPathExists(from, fp) == false)
	end

	local prefix = ("|cffffcc00%s|r"):format((ns.L and ns:L("PRINT_PREFIX")) or "MH")
	if blocked then
		-- Nothing honest to say about flying: no shared network between here and there.
		-- The portal branch above already had its chance, and silence beats a wrong hop.
		return
	elseif from and from ~= fp then
...
function ns.RouteFirstToFlightPoint(targetMap, x, y, name, currentMap)
	...
	--- 🔴 IS THERE ACTUALLY A FLIGHT BETWEEN THESE TWO? Until 3 Sep 2026 nobody asked, and
	--- the answer was often no. Rob, in Harandar, routed to Zul'Aman: "Fly from The Den to
	--- Torntusk Overlook." MEASURED in Zygor's taxi graph: Harandar is a closed five-node
	--- star with zero outbound edges (component 35), Torntusk Overlook is on the Eastern
	--- Kingdoms network (component 1). That flight cannot be taken from there at all.
	---
	--- 📌 The old data could not know. FlightPointsData is 155 flat per-map lists with no
	--- edges, so "nearest point here" + "nearest point there" was the best it could do, and
	--- it produced an impossible instruction every time the two sat on separate networks.
	--- `Modules/FlightNetworkData.lua` (generated by tools/build_flight_network.py) carries
	--- connected-component numbers so this is now a fact rather than a hope.
	---
	--- ⚠️ nil MEANS UNKNOWN AND MUST NOT BLOCK. Zygor's graph covers 129 of our 649 points,
	--- so most pairs answer nil. Treating nil as "no path" would silence the flight hint
	--- almost everywhere and look exactly like the feature being broken -- the failure this
	--- addon has a rule about. Only a hard `false` stops us.
	if ns.FlightPathExists then
		local ok = ns.FlightPathExists(fromName, toName)
		if ok == false then
			return false
		end
	end
