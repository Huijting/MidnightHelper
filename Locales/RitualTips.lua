...
merge(ns._mhLocales and ns._mhLocales.enUS, {
	...
	RITUAL_TIP_BROKENTHRONE_PHASES = "• A short instanced scenario: clear objectives and enemy waves, then a final boss, then loot the Ritual Chest.|n• Scenario seen at the obelisk: \"A Corrupted Path\" — Faithbreaker Ger'lok corrupts everything with the Void (final-boss kill still to confirm).|n• DEADLY on the later bosses (e.g. the Corrupted Amani Dragonhawk): void mirrors (Dissonant Reflections) keep spawning and cast Dissonant Realities — a massive burst on everything within 100 yards. You can't outrange it: INTERRUPT the cast and the mirror vanishes instantly. Save your kick for exactly this.|n• Corrupted Amani Dragonhawk: a Binding Nebula traps you — you CANNOT leave until you KILL the nebula itself, so burst it down the moment you're caught. Trapped players die to Volatile Plumage (the feather eruption hits for hundreds of thousands); never stand in front of the Shadowflame Breath.|n• A site may run more than one scenario layout — to confirm in-game.",
