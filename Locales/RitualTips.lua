...
merge(ns._mhLocales and ns._mhLocales.enUS, {
	...
	RITUAL_BOSS_GERLOK_STEPS = "• He summons minions that make him take (almost) no damage — burn every add the instant it spawns; the adds charge with a fire channel that nearly one-shots, so step out of it.|n• {SPELL:1273031} (Shadowbolt Volley) is THE cast to interrupt — a big AoE nuke; not every cast is kickable, so save your interrupt for it.|n• {SPELL:1279186} (Shadow Blast) is spammed — break line of sight behind the pillars.|n• Keep him ON his platform — pulling him down/off it resets the fight (Rob, live). The space is cramped, but don't chase room below.",
