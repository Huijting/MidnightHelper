...
merge(ns._mhLocales and ns._mhLocales.enUS, {
	...
	RITUAL_BOSS_GERLOK_STEPS = "• He summons minions that make him take (almost) no damage — burn every add the instant it spawns; the adds charge with a fire channel that nearly one-shots, so step out of it.|n• {SPELL:1273031} (Shadowbolt Volley) is THE cast to interrupt — a big AoE nuke; not every cast is kickable, so save your interrupt for it.|n• {SPELL:1279186} (Shadow Blast) is spammed — break line of sight behind the pillars.|n• His platform is cramped: drag him to the lower platform for room, but not too far — he resets.",
...
merge(ns._mhLocales and ns._mhLocales.nlNL, {
	...
	RITUAL_BOSS_GERLOK_STEPS = "• Hij summont minions die hem (vrijwel) immuun maken — brand elke add direct weg zodra die spawnt; de adds chargen met een vuur-channel die je bijna one-shot, dus stap eruit.|n• {SPELL:1273031} (Shadowbolt Volley) is DÉ cast om te interrupten — een grote AoE-knal; niet elke cast is kickbaar, dus bewaar je interrupt hiervoor.|n• {SPELL:1279186} (Shadow Blast) wordt gespamd — breek line of sight achter de pilaren.|n• Zijn platform is krap: trek hem naar het lagere platform voor ruimte, maar niet te ver — dan reset hij.",
...
merge(ns._mhLocales and ns._mhLocales.deDE, {
	...
	RITUAL_BOSS_GERLOK_STEPS = "• Er beschwört Diener, die ihn (fast) immun machen — brennt jedes Add sofort nieder; die Adds stürmen mit einem Feuer-Kanal, der euch fast one-shottet, also tretet heraus.|n• {SPELL:1273031} (Shadowbolt Volley) ist DER Zauber zum Unterbrechen — ein großer AoE-Schlag; nicht jeder ist unterbrechbar, hebt euren Kick dafür auf.|n• {SPELL:1279186} (Shadow Blast) wird gespammt — brecht die Sichtlinie hinter den Säulen.|n• Seine Plattform ist eng: zieht ihn für Platz auf die untere Plattform, aber nicht zu weit — sonst setzt er zurück.",
...
merge(ns._mhLocales and ns._mhLocales.frFR, {
	...
	RITUAL_BOSS_GERLOK_STEPS = "• Il invoque des serviteurs qui le rendent (presque) insensible aux dégâts — brûlez chaque add dès son apparition ; les adds chargent avec un canal de feu qui tue presque d'un coup, sortez-en.|n• {SPELL:1273031} (Shadowbolt Volley) est LA incantation à interrompre — une grosse explosion de zone ; toutes ne sont pas interruptibles, gardez votre kick pour celle-là.|n• {SPELL:1279186} (Shadow Blast) est spammé — coupez la ligne de vue derrière les piliers.|n• Sa plateforme est exiguë : tirez-le vers la plateforme inférieure pour de la place, mais pas trop loin — il se réinitialise.",
...
merge(ns._mhLocales and ns._mhLocales.esES, {
	...
	RITUAL_BOSS_GERLOK_STEPS = "• Invoca esbirros que lo vuelven (casi) inmune al daño — quema cada add en cuanto aparezca; los adds cargan con un canal de fuego que casi te mata de un golpe, así que sal de él.|n• {SPELL:1273031} (Shadowbolt Volley) es EL lanzamiento a interrumpir — un gran estallido de área; no todos son interrumpibles, guarda tu patada para ese.|n• {SPELL:1279186} (Shadow Blast) se lanza sin parar — corta la línea de visión detrás de los pilares.|n• Su plataforma es estrecha: arrástralo a la plataforma inferior para tener espacio, pero no demasiado lejos — se reinicia.",
...
merge(ns._mhLocales and ns._mhLocales.ptBR, {
	...
	RITUAL_BOSS_GERLOK_STEPS = "• Ele invoca lacaios que o deixam (quase) imune a dano — queime cada add assim que surgir; os adds avançam com um canal de fogo que quase te mata de uma vez, então saia dele.|n• {SPELL:1273031} (Shadowbolt Volley) é A conjuração para interromper — um grande estouro em área; nem toda é interrompível, guarde seu kick para ela.|n• {SPELL:1279186} (Shadow Blast) é spammado — quebre a linha de visão atrás dos pilares.|n• A plataforma dele é apertada: puxe-o à plataforma inferior por espaço, mas não longe demais — ele reseta.",
