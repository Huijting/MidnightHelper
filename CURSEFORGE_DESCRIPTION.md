...
- **Party targets** — a small panel showing every group member and what they are attacking, with their role, their class colour and the raid marker sitting on their target. Each row is split in two: **left half is the person, right half is what they are looking at. Left-click either to target it; right-click the left half to dispel them and the right half to purge their target.** The row turns red when the game says there is something on that person you can remove — Blizzard draws that, so it is never our guess about what an aura is. All of it works in combat. Drag it anywhere, drag the corner to widen it. Off until you switch it on with `/mh partytargets` or in Settings. ⚠️ **New in 3.6.0 and partly unproven:** the red and the purge have been confirmed in a live dungeon; the dispel click itself has not yet been caught working on a real debuff. Reports welcome — `/mh glow` prints what your client actually did.
