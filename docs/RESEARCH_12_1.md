...
| `docs/PTR_COILED_ISLE.md` | Coiled Isle rares, treasures and vendors with ids + coords | measured, 2026-08-06 |
...
### The Coiled Isle is mapped (6 Aug 2026)

One PTR lap with the vignette recorder produced the zone's contents: 8 rares, 12
treasures, 4 lore objects and 2 vendors on map 2512, each with an npcID and
coordinates. Full dataset in `docs/PTR_COILED_ISLE.md`.

**Conclusion that matters here:** the isle is an ordinary outdoor zone, not a raid
antechamber — it carries the full spread of rare and treasure types. So the zone
scaffold can be built from measured data instead of waiting for the patch, and
`MAP_TO_ZONE_KEY` can gain 2512 in the same change that brings the content.

Still to verify on live after the 11th; a lap is not a census.
