...
- ⚠️ **What that deliberately no longer answers:** worn gear tells you which set you are WEARING, not which set is yours this season — Rob's Shaman wears last season's tier, so his gear says "Mantle of the Primal Core". There is no client source for the other question. Written into `TierSetData.lua` along with the instruction not to reintroduce a table to close the gap.
- 🔴 **`fill()` was undoing the "do not translate proper nouns" rule on every load, and following the rule could not stop it.** It replaces any value identical to enUS, because the packs copy every English string at load and an `is it nil` test could never fire — that test rescued ~400 real translations per language and has to stay. It also cannot tell a placeholder from a key left English ON PURPOSE: `itIT.lua:1012` correctly kept "Veteran of the Dawn" and `Translations2026.lua:7376` turned it into "Veterano dell'Alba".
- **So intent became data.** `Locales/KeepEnglish.lua` holds keys Blizzard owns in every language, plus a per-language half for answers that differ; both fill files consult it and **lint check [15]** fails when a pack carries a different value. Verified by breaking one value and watching the check fire, then restoring byte-for-byte — a check that reports zero proves nothing.
- **Crest ranks are English in `nlNL`, and that is a rule rather than a preference.** Carola went looking for "Kampioen crest" and found nothing: there is no Dutch WoW client, so a Dutch player always reads *Champion Crest*. ⚠️ Explicitly NOT decided for esES/ptBR/itIT, which translate it today while deDE/frFR do not — that needs someone running those clients. To #translations.
- **`DAWNCREST_TIER_ADVENTURER` split into a name and a hint.** "Adventurer (green)" was a name Blizzard owns plus a clarifier of ours, and the guard compares whole strings, so listing it fired on the CORRECT Dutch value. Loosening the check to a substring test would have let "Avonturier (groen)" through everywhere. Packs were cut mechanically at the parenthesis — nothing translated, only moved.
...
- 🔴 **The right-click dispel never worked, and 3.6.0 shipped it announcing that it might not.** The button carried the spell ID; for a spell a spec replaces — Purify on a Holy Priest — the base ID resolves to nothing, so the click cast nothing and reported nothing. `CastableForm` now passes the NAME, as HexBreak does (`Core.lua:1731`). ⚠️ This is a deliberate exception to this repo's own "cast by ID, never by name" rule, which stays everywhere else: a renamed pet breaks a name, and a spec-replaced base ID breaks an ID. Different failures, both real. Rob confirmed dispel *and* purge in a live dungeon.
- 🔴 **What actually found it was elimination, not thinking harder.** Four clicks — left/right × red row/plain row × in/out of combat — killed three standing theories in ten minutes: the glow eats nothing, combat is not the cause, and the star-prefixed attribute form resolves fine. Everything before that was explaining behaviour nobody had bounded. **And the answer had been in HexBreak the whole time**; that file was opened three times, once per question, and line 1731 was there on all three visits.
- **Range fading** (`PartyTargets.lua`): a row dims when your dispel cannot reach that unit. Measured with the dispel itself rather than a stand-in spell, so it inherits the real range including talents. Method is DandersFrames' `Features/Range.lua`, which is Grid2's — phased first, then `C_Spell.IsSpellInRange` (normal booleans, not secret values), then `CheckInteractDistance` with the out-of-combat leniency both of them gate the same way. Every unknown resolves to full brightness: a wrongly dimmed row says "do not bother" about someone you could have saved.
- **A row for the player**, gated on actually having a dispel. Rob asked whether it could appear only when something is dispellable; detecting that is possible (your own auras stay readable), showing it is not — a secure frame cannot be shown or hidden in combat, so the row would arrive after the fight. Having a dispel at all changes only on a spec or loadout swap, neither of which happens in combat, so that gate is safe. ⚠️ `UNIT_AURA` was registered for party1-4 and not for `player`; without that the new row's indicator would only have updated when somebody *else* gained an aura.
- 🔴 **And that row shipped broken to Rob within the hour: it drew BELOW the panel.** The panel is exactly as tall as it has visible rows, which works because absent members sink to the bottom and the visible ones stay packed. Pinning the player to index 5 broke that promise for a party of two. It takes the first free index now. Built and tested against four followers, where index 5 is accidentally correct; two people knocked it over immediately.
- **A click no longer waits for you to stand still** (`Delves.lua`): a full refresh is deferred while `GetUnitSpeed("player") > 0`, which is right for currency/quest/POI bursts and wrong for something the player just pressed — and on a flight path you never stop, so the Delves sections stayed shut until landing. Clicks carry a `userAction` flag that skips the move-defer and keeps the 0.8s throttle. Set before the `pending` early-return, or a click landing on an already-queued run is swallowed silently.
- **The Tier Sets page says its data is Season 1's**, because it is: `TierSetData.lua` names and bonus IDs come from a 12.0.7 datamine researched on 16 June. The footer used to hedge ("may still be last season's"), which was honest before anyone checked and too soft afterwards.
- 🔴 **Season 2 DOES have tier sets — that conclusion was one step from being recorded backwards.** Three measurements all said no set pieces in the raid: none in the journal loot table, no set entry under Ula'tek, identical lists per armour type. All three are what a token system looks like from outside. Bosses drop tokens; Kirana next to the Catalyst trades them (official patch notes). **The tokens were in our own capture** — Venomwoven/Venomcured/Venomcast/Venomforged, five each plus the omni `Slumbering Coil Curio` (270909) — and the analysis discarded all fifty slotless items because a token has no slot.
- 🔴 **`ReadItemSetLine` was declared broken on a control that was not one.** The 25 Aug run read 115 journal items, saw no set line on any, and concluded the reader was blind — with The Voidspire as the positive control, "a Season 1 raid that certainly had tier sets". Midnight's sets are token goods, so neither season's set items are in journal loot at all: those 115 were never set pieces and every "no" was correct. `/mh setline` now runs a real positive *and* negative control; measured 28 Aug, the reader works.
- **New diagnostics:** `/mh setline` (above), `/mh tierscan` (raid loot per class, since clearing the class filter is what hid the tokens — with a printed item count per class so an ignored filter is visible instead of believed), and `/mh range` (what the fade decided and why).

## 3.6.0

- 🔴 **Valeera: kills feed her, and we said they did not.** Rob measured +104 standing from a run with no qualifying loot at all. The popup said "picked up", full stop, so anyone who followed us skipped packs for nothing. `LogLootWindow` now records `ambiguous` when a window holds more than one candidate, because a gain that cannot be attributed must not be attributed.
- 🔴 **The Catalyst inherits your stats since 12.1, which inverts the old advice.** Blue post (18 jun) and the `Venomblight Manaflux` tooltip (currency 3465) both say so. It used to stamp fixed stats on whatever you fed it, so converting a badly rolled leftover was the clever play; now a bad piece comes back bad, permanently. The tier guide leads with the CHANGE rather than the current rule — someone told only what is true today learns a rule, someone told it changed re-examines what they already do. ⚠️ Blizzard writes "certain special cantrip effects"; the hedge is kept. `TierSet.lua` itself was never the gap — the text lives in the locale, which is why a grep of the module reported one.
- **Manaflux is per character, cap 8** (`VENOMBLIGHT_MANAFLUX = 3465`). Assumed account-wide from a single data point; Rob's own tooltip disproved it. Stored as `nil` rather than `0` when unknown, so "no data" and "none left" stay distinguishable. The counter went into a row tooltip first and he could not find it — it is in the account summary block now.
- **`/mh stats`** (new, `Modules/StatCoach.lua`): the curated per-spec order from `vault_stat_priorities.json` had been driving the Vault advisor for months and the only thing a player ever saw of it was one line of jargon. Mastery text comes from the client, per spec, already translated — forty descriptions we would otherwise have to maintain. Measured on live: `C_SpecializationInfo.GetMasterySpells` does not exist, `GetSpecializationMasterySpells` does; the loser is deleted rather than kept "just in case". Equal weights share a rank, because a number is a claim and haste = crit for Elemental.
- **Party targets dispel and purge** (`PartyTargets.lua`): the row is split into two secure buttons — left half the member (right-click dispels him), right half his target (right-click purges it). Rob's own observation: the screen was already in two halves, only the click areas were not. Blizzard draws the red via a `CustomAuraContainerTemplate` on `HARMFUL|RAID`; we never read the aura, which is what makes it legal in 12.1.
- 🔴 **And the artwork took four attempts, all the same mistake.** Icon-sized, then hung on a zero-sized slot, then raised eight frame levels — each moved the art while leaving it anchored to a frame Blizzard controls, and each drew a 2px line. The measurement that ended it: at paint time everything is already right (`slot 0x0 -> 362x22`, level 9 against panel 1, no swallowed error), so the container shrinks *afterwards* and `SetAllPoints` follows it down. The art now takes its rectangle from our own panel and only its PARENT stays the slot. **When geometry is correct as you set it and wrong when drawn, remove the dependency instead of moving the art again.**
- 🔴 **Sizing the slot to the row kills every click**, twice discovered. It covers the secure button and `SetMouseClickEnabled(false)` does not survive the engine showing it. The art sits on a mouse-disabled canvas parented to the slot; the slot keeps whatever size Blizzard gives it.
- **`/mh glow test` and `/mh dispeltest`**: three failed builds cost a `/reload` each *plus* a boss that might not cast anything relevant, while the answer was visible standing still. Both say out loud what they do NOT cover — the glow test cannot make Blizzard show a slot, and `dispeltest` cannot manufacture a secret value, so it says so instead of faking one.
- **A ritual site is a group with nobody in it, and a scenario that is not a delve.** Three modules asked a question whose usual answer is right everywhere else: `IsInGroup()` returns true there (measured: `allyPresent=false, IsInGroup=true`), so BUFF ALLY appeared with no one to buff; rituals are scenarios, so Valeera's window stayed open saying "This delve"; and the route arrow hung in the air at 248m. The arrow is now HIDDEN inside instances, never released — clearing a route on a zone change is the bug `NativeArrow` was rewritten to stop.
- **Season stats printed two of five counters** and the missing three read as "not tracked" rather than "zero". All five print, in words rather than variable names. The command list also pointed "Season stats for this character" at the developer diagnostic; it names `/mh season stats` now.
- **Windows honour their own settings** (thanks **AndyMM22**, PRs #1 and #2): the boss window ignored its off switch outside dungeons, and a group member's request opened the consumable board regardless of the receiver's choice. Four panels stopped working while hidden: 0.060 → 0.003 ms per frame idle.
- **Every dialog can dock to the main window**, with a button to release, and drag-to-release because a window that follows the cursor while still counted as docked snaps back later. The share/copy dialog predated `DialogPopup` and had never registered, so it had been missing Shift+scroll scaling all along — one line, five windows.
- 🔴 **Four EditBoxes could keep the keyboard after closing.** Found by reading WoWNext's 2.0.3 hotfix, whose own note is the useful half: *"No WoWNext key binding to A was found; the issue was caused by retained EditBox focus."* All twenty of our boxes call `SetAutoFocus(false)`, which stops them TAKING focus and does nothing about giving it back. Rob has never lost a key to it; it was latent.
- **`|c%x+` is greedy and A-F are hex digits**, so the colour-strip in `CurioExplain` ate the start of the highlighted word. Proven outside the game: `Blood Shield` → `lood Shield`, `deflect` → `lect`. Exactly eight digits now, as `DelvePartyShare` already did.
- **48 delve story variants** (`DelveStoryData.lua`, from DB2), where Collegiate Calamity alone had three and the client already knew four. ⚠️ Our hand-written list was never the source — the delve POI carries `Story Variant: <name>` and Rob's SavedVariables held eleven measured ones.
- **Translation drift is measurable** (`tools/check_drift.py`): `ns:L` falls back to enUS when a key is MISSING, never when it is present, so correcting an English sentence leaves five packs answering the old question. Seven keys were loose, `VALEERA_RUN_FMT` among them — still carrying the claim withdrawn on 20 aug. ⚠️ The checker does not read Lua: a static parser was wrong three times while it was being built, and `locale_probe.lua` contradicted it every time. Four wrong `SET_CONSREADY_TOGGLE_TITLE` translations were DELETED rather than left standing; a missing key falls back to the corrected English.
- 🔴 **`UNIT_SPELLCAST_INTERRUPTED` was measured, and 18 aug still stands.** CastBreaker builds an interrupt history on it, so the closed question was reopened rather than defended. 658 events on live: your own interrupted casts are readable (`Smite`, `Holy Fire`), every enemy's is `SECRET` on nameplate, party and targettarget, and `interruptedBy` is nil in all 658. Their history cannot work on this client.
- **Secret-value guards** in `GroupBuffs` and `CombatSafety`, prompted by CastBreaker throwing `attempt to compare local 'matches' (a secret boolean value)` 145 times on Rob's screen. `pcall` is not the guard — calling is allowed, comparing throws. An unreadable "is he dead" now counts as alive, because dropping a member we cannot read would turn "3 of 5 buffed" into a confident "3 of 3".
- ⚠️ **Left open deliberately:** one clean right-click on a RED row, with no other key pressed, has never been done. The cast log records every spell from any source, so a run of `Dispel Magic (528)` during a boss fight proved nothing about which button was clicked — a conclusion drawn from it, and retracted.

## 3.5.0

- **All five packs completed**, ~30k characters each: the professions course, the Codex, dungeon and delve guidance, the academy, the commands and the achievement text. Split across per-language commits so a single language can be reverted without touching the other four. ⚠️ The linter's parity line reads 89–91% and that is not the gap it looks like: 311 of the ~300 remaining keys per pack are `CHANGELOG_*`, English by design. Measuring sentences instead (≥4 real words) leaves **zero** in all five — the last one was `WAY_PORTAL_HINT`, written the same evening and translated before the tag. The rest are stat names, `DPS`, `Flask` and format strings.
- **Game terms checked against Blizzard's DB2** (`wago.tools/db2/<Table>/csv?build=<build>&locale=<code>` — build-pinned, no rate limit, and harder than Wowhead because it is the client's own table). Four were wrong: itIT called Deftness `Destrezza` where the Italian client says `Velocità`, and `Crafting Details`, `Patron` and `Recipe Difficulty` sat in English inside translated frFR/esES/ptBR sentences.
- ⚠️ **A present key does not fall back, so correcting enUS silently leaves five packs wrong.** Seven strings were stale, including the work orders chapter still claiming four public orders *per day* in five languages — the exact error 3.4.0 announced fixing — and the Alchemy and Herbalism chapters still describing the routes 3.4.0 replaced. This is the failure mode to watch after every content correction: `fill()` in `Translations2026.lua` is fill-only by design and cannot repair it.
- **`tools/translation_todo.py` now asks the resolver, not the file.** It runs `locale_probe.lua` and reads what `ns:L` returns per language. It also gained `--prefix`, to finish an *area* rather than a release — the `--since` question is blind to strings older than the tag, which is how 46 professions strings sat untranslated behind a report of "complete". And a `SETTLED` table records which strings are English on purpose with the reason for each, because a verbatim English copy and a missing translation are indistinguishable from the resolver's side: that ambiguity produced a "400 characters remaining" report where all 18 keys were already correct.
- **`Modules/ValeeraProgress.lua`** (new): delve-only popup on `C_GossipInfo.GetFriendshipReputation(2744)`, name-checked against "valeera" rather than trusting the ID alone. Reports bountiful XP measured from the player's own last 8 runs — kept as recent samples rather than a running average, so a changed rate shows up instead of being diluted.
- 🔴 **The run estimate was CUT before release, and the premise was false.** It said "only Bountiful delves add to it (measured 2026-08-20)" and turned the remainder into a number of bountiful runs. Rob disproved it from in-game: a level 3 delve, not bountiful, gained Valeera reputation — less than a bountiful run, but not nothing. The 20 aug check had confirmed that bountiful delves DO feed her and concluded the others do not: absence inferred without a positive control, the project's most-repeated mistake, shipped in six languages. His second point retired the feature rather than the sentence — he does not believe the per-delve gain is reliably measurable, and predicting from a quantity that cannot be pinned down is false precision. `BountifulStats`, `bountifulGains`, `enterSnapshot`/`OnLeftDelve` and the `IsDelveBountiful` hookup are all deleted rather than patched; dead bookkeeping nobody reads is how this rots. One true sentence remains: every delve adds to it, bountiful ones add more. ⚠️ Do not rebuild the estimate without a real measurement: same tier, bountiful and not, several runs, gains written down.
- **The X is a real off switch.** It set `_userDismissed`, which reset on leaving, so anyone who did not want the popup had to close it every single delve — the same "always on screen" complaint the feature was built to answer. It now persists in `ns.db.valeera.off`, `ns.ToggleValeeraPopup` clears it, and closing prints how to get it back, because a setting nobody can find is the same as no setting. The entry snapshot is still taken while hidden, so turning it back on finds a real measurement waiting.
- ⚠️ The reworded strings went into each language's OWN section of `Translations2026.lua`, not a fresh block at the top: `fill()` is fill-only, so an earlier copy wins and silently kills the checked one below (the linter caught it, 10 HARD). It also matters per language — German says `Tiefen`, French `Gouffres`, the rest keep `delve`; one block written in a single sitting would have flattened all three.
- **Travel advice knows about portals.** `ReportTravelHintForWaypoint` asks `ns.BuildTravelPlan` before speaking, so a portal is reported as the first step. `RouteFirstToFlightPoint`'s stand-down re-points the waypoint instead of leaving nothing leading, and `StartLegWatcher()` was extracted so the portal leg arms the same hand-off the flight leg already had. ⚠️ Its `pendingLeg` has no `toName` — that field is the flight-arrival test, and setting it would make the portal leg wait for an arrival that never fires.
- **Two promises to players retired.** `CONSREADY_FOOTER` said bag sharing was "coming in Phase 2" — it shipped in **1.8.3** (`ConsumableReadyComms.lua`, `MHCons` prefix, `CHANGELOG_183_9` announced it). The footer told every player the opposite of what the addon does, in seven packs. And `RITUAL_BOSS_MINDBREAKER_STEPS` promised detail "in the next Midnight Helper update" across 3.2.0, 3.3.0, 3.4.0 and 3.5.0; it now states the actual position and asks for a report. ⚠️ A promise in a locale string has no expiry and nothing checks it — when writing one, prefer a description of the current state over a schedule.
- **Valeera's popup counts what feeds her, measured on 24 aug.** Chunks of Companion Experience (14 ids confirmed in the client via `/mh chunks`; rarity read from the item's own `quality`, never a copied table), Boons, and a 26,000-XP tier of find (`Insect Shedding`, `Rootlight Lamppost`). ⚠️ Extras are counted **by effect**: a loot event containing no chunk that still raised her standing. A name match on "Boon of" would work for one locale and fail silently in five, and an id list goes stale on the next patch. Only unambiguous events count — an event with a chunk in it cannot be attributed, which is the error the first per-item log made (19 chunks and 19 "other sources" with identical amounts, one gain counted twice).
- **The course is one surface.** `ProfessionAcademy` no longer renders chapters; the tick, task and waypoint moved to `ProfessionCourseWindow` via `ns.MH_SetChapterDone` / `ns.MH_RouteChapterWaypoint`. Panel −248 lines; rail, row builder, chapter-position map, seven locals and `PROFACAD_CONTENTS_SHOW/_HIDE` deleted with it. The window's rail now takes 36% of the width (floor 172, ceiling 320) and updates in `OnSizeChanged`.
- 🔴 **`CanSpendKnowledge` had one reader.** The 21 aug fix landed in `ProfessionNextStep` only, so `BuildAdviceLine` still named a root on a fully padlocked profession. Exported as `ns.MH_CanSpendKnowledge`; the node list is suppressed while locked too. **Fourth bug this week from two places answering one question.**
- 🔴 **`ConsumableReadyBoard`: secure buttons were parented to the board.** `ADDON_ACTION_FORBIDDEN` on `CastSpellByName`, latent since 1.8.4 and intermittent because taint depends on what happened to the board before the click. Both button sets moved to `UIParent` and *positioned* from its screen rect (`PlaceSecure`/`PlaceAllSecure`), never anchored — `MissingBuff.lua:606` carries the same warning after paying for it once. ⚠️ Placement runs after `Show()`: an unshown frame has no `GetLeft()`.
- **The Ring of Glory measured end to end** (Tier 11, solo): Crushfoot → Bluegill Brothers (Lurgle + Smurgle + Tagteam) → Brinebeater → Guth'kar the Bound → 3× Arena champion → Drakta. Tip rewritten in 7 languages around "the series varies". Crushfoot CC = measured; the blue-orb towers = three independent player reports, labelled as such. ⚠️ `Soul Cleave` and the pillars are now measured; the Death Grip "break line of sight" line is **removed** as unfollowable solo.
- **Venomfall Deeps shipped** (`8779`, maps 2512 and 2437). The 14 aug note recorded measured absence and named the re-test; Season 2 opened, `/mh atal` was re-run, the client returned it. Both maps carried — the roster is keyed by map.
- **PartyTargets rows are people.** Left targets `party<i>`, right `party<i>target`, both via `unit`/`unit2` on one scriptless button. ⚠️ Rebound together on re-sort. First build had it reversed; Rob tested it in a delve within the hour and the platform convention won.
- `PLAN_ALREADY_THERE` said "close enough that a plan would be silly" when it meant "no travel steps were generated" — 405 yd across Silvermoon is not close. 7 languages.
- ⚠️ **OPEN, unproven:** `OneContainsTheOther` (via `GetMapInfo().parentMapID`) was meant to stop "other continent" firing for Silvermoon City → Eversong Woods. It did not change the message and is currently **inert**. `/mh arrow` now prints both maps, both continent ids and both parent chains so one screenshot names the culprit. ⚠️ Which direction was tested is unknown — from Harandar "travel back" is *correct* and measures nothing.
- **`tools/bash_guard.py`** (new, dev-only): a PreToolUse hook refusing shell commands containing `&&`, `||`, `;`, pipes, heredocs or `python -c` — shapes no permission rule can match, which is why 86% of Bash calls prompted regardless of how many rules existed. Quoted text is stripped before matching so a commit message may contain `&&`. Registered in `AddOns/.claude/settings.json`, which sits above this repo and is therefore not version-controlled.
