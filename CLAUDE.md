...
- `RELEASE_NOTES.md` (repo root) — **the CurseForge release notes**. The packager uploads it *verbatim* (`.pkgmeta` → `manual-changelog`, `markup-type: markdown`), so it must hold **only the current release**, pure Markdown starting with a plain `# heading`, and stay identical to `docs/CURSEFORGE_<ver>.md`.
  - ⭐ **KEEP IT SHORT — under ~40 lines and without bullet lists.** For months the auto-upload arrived backslash-escaped with its newlines collapsed (`\## Know your role`) and Rob had to paste it by hand every release. Four releases settled it:
    ...
    📌 **3.2.0 narrows it further, and against character count.** At **2341 characters** it is the longest clean upload yet — longer than one of the mangled ones — while sitting at only 27 lines with no bullets. So neither line count nor character count is the trigger on its own. It also went up **twice** (Rob withdrew the first while checking something unrelated) and rendered clean both times, which rules out per-upload flakiness.

    🔴 **CORRECTED 20 Aug 2026 — two sentences above were wrong about the table above them.** They claimed no clean upload had ever carried a bullet. Recounted from the archived files themselves (`grep -c "^[-*] " docs/CURSEFORGE_*.md`): 2.8.2 nine bullets ❌, **2.8.3 one bullet ✅**, 2.8.4 six ❌, 2.9.0 zero ✅, 3.2.0 zero ✅. A bullet *has* appeared in a clean upload. The bullet theory is weakened, not confirmed, and it was being restated as fact each release.

    🔬 **AND THE STREAK IS CONFOUNDED — 3.3.0 IS THE EXPERIMENT.** The last mangled upload was 20 Jul 2026. Everything since is clean, but two things changed at once: we started writing short, *and* `.github/workflows/release.yml` pins `BigWigsMods/packager@v2`, a floating tag whose code can change under us silently. "Our rule works" and "the uploader was fixed in July and the rule has done nothing since" fit every data point equally. Care cannot separate them; only a deliberate violation can.

    So **3.3.0 reproduces the failing shape on purpose: 56 lines, 4545 chars, 9 bullets** — longer on every axis than either mangled release. Read the result before writing another line of this section:
    - **Clean** → the rule is dead, the cause was upstream, and release notes may be written properly again. Delete the rule; keep the table as history.
    - **Mangled** → the rule is real and not a coincidence. Restore "under ~40 lines, no bullets" as binding, and record that 4545/56/9 fails while 2341/27/0 passes.
