...
- `Locales/Translations2026.lua` adds post-2025 translations for **de/fr/es/pt/it** through `fill(code, patch)`. Add new translations here.
  - 🔴 **`fill()` DOES OVERWRITE — this line said "only if the pack lacks it" and was wrong.** Measured 10 Sep 2026 (Spec 38 §1c, `Translations2026.lua:44`): a key is set when the pack's value is `nil` **or identical to the enUS text**. The packs copy every enUS string at load, so a pure "fill if missing" would never fire - the comment above `fill()` says so, with numbers. What it never touches is a value that differs from English (someone's real translation) or a key listed in `Locales/KeepEnglish.lua`.
  - ⚠️ So an English value in a pack is **not** a way to keep a term English: `fill()` replaces it. Declare it in `KeepEnglish.lua`. Spec 38 found the effect on screen: the relog badge became "(neu einloggen)" in German, and the vault letters "M D R" in French and Spanish.
